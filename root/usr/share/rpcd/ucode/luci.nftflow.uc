...
function firewall_read() {
    ...
    if (config != null)
        return { ok: true, config, path: SAVED_FIREWALL, customized: true };
    config = read_text(DEFAULT_FIREWALL);
    return config == null
        ? { ok: false, error: `cannot read ${DEFAULT_FIREWALL}` }
        : { ok: true, config, path: DEFAULT_FIREWALL, customized: false };
}
...
const methods = {
    ...
    diagnostic_dns: {
        ...
        call: request => {
            ...
            return defer_ctl(request, [ 'diagnostic-dns', args.source || '', args.domain || '' ], 'DNS diagnostic');
        }
    },
    diagnostic_request: {
        args: { domain: '' },
        call: request => defer_ctl(request, [ 'diagnostic-request', request_args(request).domain || '' ], 'Request diagnostic')
    },
    diagnostic_firewall: {
        args: { address: '' },
        call: request => defer_ctl(request, [ 'diagnostic-firewall', request_args(request).address || '' ], 'Firewall diagnostic')
    },
    action: {
        args: { name: '' },
        call: request => {
