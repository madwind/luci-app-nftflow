...
function geoip_path() { return uci_get('geoip_file', ''); }
...
function compile(raw) {
    ...
    let path = geoip_path();
    if (!path) return { ok: false, error: 'GeoIP database path is not configured' };
    if (substr(path, 0, 1) != '/') return { ok: false, error: 'GeoIP database path must be absolute' };

    let cache = {}, buckets = {}, bucket_order = [], output = '', position = 0;
...
function runtime_current() {
    ...
    for (let spec in managed.tables) {
        ...
        if (!listed.ok)
            return { ok: false, error: trim(listed.output || '') || `unable to read nftables table ${spec.family} ${spec.name}` };
...
function validate(raw) {
    ...
    let source = template.source, built = compile(template.rendered);
    ...
    if (!checked.ok)
        return { ok: false, valid: false, error: 'nftables syntax check failed', detail, warnings: built.warnings };
    return { ok: true, valid: true, config: source, compiled: built.compiled, warnings: built.warnings };
}
function save(raw) {
    ...
    return saved.ok ? { ok: true, valid: true, config: checked.config, warnings: checked.warnings } : { ok: false, error: saved.error };
}
...
function apply(raw) {
    ...
    let desired = checked.compiled;
    let loaded = run_transaction(transaction(managed.tables, desired));
    ...
    if (!source_saved.ok)
        return fail_open(source_saved.error || 'cannot save applied firewall snapshot', 'nftables runtime was removed after the snapshot save failed');
    return { ok: true, applied: true, config: checked.config, warnings: checked.warnings };
}
...
function dispatch(command, args) {
    if (command == 'firewall') {
        ...
        if (source == null) return { ok: false, error: `cannot read ${FIREWALL_SOURCE}` };
        return apply(source);
    }
