...
const DEFAULT_SOURCE = '/usr/share/nftflow/defaults/routing.conf';
...
function default_state() {
    ...
    if (raw == null) return { ok: false, error: `cannot read ${DEFAULT_SOURCE}` };
    let parsed = parse_config(raw);
    return parsed.ok
        ? { ok: true, state: parsed.state, path: DEFAULT_SOURCE, customized: false }
        : { ok: false, error: `invalid default routing file: ${parsed.error}` };
}
function effective_source() {
    let raw = read_text(SOURCE);
    if (raw != null) return { ok: true, raw, path: SOURCE, customized: true };
    let fallback = read_text(DEFAULT_SOURCE);
    return fallback == null
        ? { ok: false, error: `cannot read ${DEFAULT_SOURCE}` }
        : { ok: true, raw: fallback, path: DEFAULT_SOURCE, customized: false };
}
...
function read_current() {
    ...
    let parsed = parse_config(source.raw);
    if (!parsed.ok) return { ok: false, error: parsed.error };
    return { ok: true, config: parsed.state.normalized, path: source.path, customized: source.customized };
}
...
function save(raw) {
    ...
    if (parsed.state.normalized == fallback.state.normalized) {
        fs.unlink(SOURCE);
        return { ok: true, valid: true, config: fallback.state.normalized, customized: false };
    }
    let result = atomic_write(SOURCE, parsed.state.normalized);
    return result.ok
        ? { ok: true, valid: true, config: parsed.state.normalized, customized: true }
