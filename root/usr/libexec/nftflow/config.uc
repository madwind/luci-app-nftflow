...
function apply(raw) {
    ...
    if (!restarted.ok)
        return { ok: false, saved: true, path: resolved.path, exists: !!config, config, error: 'configuration was saved but NftFlow restart failed', detail: trim(restarted.output || '') };

    return { ok: true, saved: true, applied: true, path: resolved.path, exists: !!config, config, detail: trim(restarted.output || '') };
}
